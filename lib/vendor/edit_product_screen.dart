...
class _EditProductScreenState extends State<EditProductScreen> {
  ...
  void populateFields() {
    ...
    selectedBrand = widget.productData['brandId'];

    // Existing Image
    if (widget.productData['imageBase64'] != null && 
        widget.productData['imageBase64'].toString().isNotEmpty) {
      existingImageBase64 = widget.productData['imageBase64'];
      imageBase64 = existingImageBase64;
      try {
        selectedImage = base64Decode(existingImageBase64!);
      } catch (e) {
    ...
    descriptionController.text = specs['description'] ?? '';
    
    // Hardware
    processorController.text = specs['processor'] ?? '';
    ramController.text = specs['ram'] ?? '';
    storageController.text = specs['storage'] ?? '';
    graphicsController.text = specs['graphics'] ?? '';
    
    // Display
    displaySizeController.text = specs['displaySize'] ?? '';
    resolutionController.text = specs['resolution'] ?? '';
    refreshRateController.text = specs['refreshRate'] ?? '';
    displayTypeController.text = specs['displayType'] ?? '';
    
    // Battery & Physical
    batteryController.text = specs['battery'] ?? '';
    weightController.text = specs['weight'] ?? '';
    dimensionsController.text = specs['dimensions'] ?? '';
    
    // Connectivity
    portsController.text = specs['ports'] ?? '';
    connectivityController.text = specs['connectivity'] ?? '';
    
    // Software & Warranty
    osController.text = specs['os'] ?? '';
    warrantyController.text = specs['warranty'] ?? '';
    
    // Additional
    webcamController.text = specs['webcam'] ?? '';
    keyboardController.text = specs['keyboard'] ?? '';
    audioController.text = specs['audio'] ?? '';
    ...
    try {
      ...
      if (image != null) {
        ...
        setState(() {
          ...
          existingImageBase64 = null; // Clear existing reference
        });
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    if (isLoadingData) {
      return Scaffold(
        ...
        appBar: AppBar(
          title: Text('Edit Product', style: TextStyle(color: AppColors.lightGold)),
          backgroundColor: AppColors.deepBlack,
    ...
    return Scaffold(
      backgroundColor: AppColors.deepBlack,
      appBar: AppBar(
        title: Text('Edit Laptop', style: TextStyle(color: AppColors.lightGold)),
        backgroundColor: AppColors.deepBlack,
        iconTheme: IconThemeData(color: AppColors.lightGold),
      ),
      body: SafeArea(
                        ...
                        child: (selectedImage != null)
                            ...
                            : Column(
                                ...
                                children: [
                                  ...
                                  SizedBox(height: 12),
                                  Text('Tap to change image', style: TextStyle(color: AppColors.softGold)),
                                  SizedBox(height: 4),
  ...
  // --- Helper Widgets (Same as AddProductScreen) ---
